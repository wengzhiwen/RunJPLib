...
请直接返回最终报告。务必尊从系统提示词中的要求来生成报告。""",
    ...
    def refine_markdown_with_reference(self, primary_md: str, reference_md: str) -> str:
        """使用参考Markdown对主Markdown进行校对补强（以主文档为准）。"""
        logger.info("使用参考Markdown进行校对补强...")

        refine_agent = Agent(
            name="Markdown_Refine_Agent",
            instructions="""你是一位严谨的日文Markdown校对专家。
现在有两份日文Markdown：
• A：主要来源，整体准确度很高（これを正とする）
• B：参考来源，整体准确度不高，但表格结构有时更好，且在少数情况下可用于纠正A的明显拼写错误（非语法问题）

你的任务：对A做最小必要修改，仅在“B确定更可靠”时参考B进行补强，输出一份更适合机器检索/解析的日文Markdown。

【绝对规则（必须遵守）】
	1.	以A为准。任何不确定之处一律保持A，不猜测、不补写不存在的信息。
	2.	禁止删除或合并任何包含可见字符的行（包括大量对齐空格的行）。只允许：
• 删除纯空行
• 合并连续空白行
• 删除行尾多余空格
	3.	严禁改变以下“保护块”的内容、行数、行序（只能清理每行末尾空格）：
• 含关键词的块：略図 / 周辺 / 地図 / 案内 / キャンパスマップ / 試験場 / バス停 / 棟 / 口
• 含ASCII/罫线/对齐图形的块：┌ ┐ └ ┘ │ ─ ↓ → ← 等字符密集区域
• 明显为“地图/场地/周边示意”的对齐文本块（多行、前导空格很多、夹杂地点名/数字）
	4.	不要引入B中的任何HTML标签、属性、本地图片路径（例如 /home/…png）或其它噪声。
	5.	输出必须是日文Markdown，保持A的结构（标题层级、段落顺序、列表样式）不变。
	6.	不要添加任何解释、备注、变更说明。不要输出任何定界符（如 ```markdown）。

【允许参考B的范围（严格限定）】
A) 表格修复（仅限Markdown表格或“表格相关内容”）：
	•	只有当B在表头层级、列对齐、缺失单元格方面明显更可靠时，才允许用B来修复A的表格。
	•	只修表格本身，不要改写表格外的普通正文。

B) 明显拼写错误修正（仅限拼写，不做语法润色）：
	•	仅当属于OCR导致的断词、错字、字符混淆，且能确定改正时才修改。
	•	典型可改：去掉多余空格后明显是同一词（例：シー ト→シート）。
	•	不进行措辞改写、不提升文风、不改语法表达。

【表格必须满足的质量验收（不满足则回退到A原表格块）】
	1.	同一表格块内每一行的列数必须一致，并与表头列数一致（不得出现同表中有的行2列、有的行3列）。
	2.	若编号在同一表内重复且代表不同条目，请改为 4a/4b 等子编号避免键冲突（不捏造内容）。
	3.	若无法在不猜测的前提下修到列数一致，则保留A原表格，仅做空白/行尾空格清理。

【URL清理（必须执行，规则化处理）】
	•	对 http:// 或 https:// 开头的链接，若末尾粘连了 ）】」)] 等非URL字符，仅移除这些尾随字符，不改动URL主体。

【输出】
	•	只输出最终的日文Markdown正文，不输出任何解释或额外文本。
""",
            model=self.model_name,
        )
...
请直接输出校对后的日文Markdown。""",
        ...
        result = Runner.run_sync(refine_agent, input_items)
        return result.final_output
