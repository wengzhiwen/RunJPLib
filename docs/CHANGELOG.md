...
## [2025-12-22] - 校对补强与Proof归档
- **PDF 上传可选参考 Markdown**：OCR 后使用参考 Markdown（B）对 OCR 结果（A）进行校对补强，输出（C）。
- **校对任务日志增强**：在校对/翻译/分析阶段写入任务日志并记录耗时，便于排查卡点。
- **Proof 归档**：每次校对生成 `proof/时间戳_大学名称`，保存 A/B/C 三份 Markdown 以便质量追踪。
