...
## [2025-09-07] - 取消后台PDF处理页面自动轮询

### ✨ 功能改进
- 将 `templates/admin/pdf_processor.html` 中的队列状态自动轮询移除，改为手动刷新。
- 在"处理队列状态"卡片中新增"刷新"按钮，点击后即时获取 `/admin/api/pdf/queue_status`。

### 🔧 影响范围
- 避免在后台页面停留时产生不必要的定时请求，降低服务器负载。

### 📚 文档
- 本条记录说明了由自动轮询改为手动刷新的变更。
